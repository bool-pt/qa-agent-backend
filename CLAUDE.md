...
- **Per-call browser reset.** The fresh `user` UUID only resets the agent's *conversation* — it does **not** reset the underlying Chromium, whose user-data-dir persists cookies and login between calls. `src/invokeAgent.ts:resetBrowserProfile` runs at the top of `doExecute` (inside the single-flight mutex) and does two things: `openclaw browser --browser-profile openclaw stop` (best-effort), then `rm -rf $BROWSER_USERDATA_DIR` (default `$HOME/.openclaw/browser/openclaw/user-data`, openclaw's on-disk convention for managed profiles — verified against `openclaw@2026.4.15`). The next agent call's first `browser navigate` lazily relaunches Chromium with an empty profile. Both steps are best-effort and logged on failure — don't fail the webhook over a dirty user-data-dir. Note: this path **cannot** be pinned via `browser.profiles.openclaw.userDataDir` in `openclaw.json` — the gateway rejects that field on managed profiles. If openclaw moves the path in a future version, override via the `BROWSER_USERDATA_DIR` env var.
